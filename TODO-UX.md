...
- [x] il faut pouvoir supprimer une session (avec confirmation), on nettoye tous les dossier d'export, on ne touche pas au dossier source, on ne garde plus aucune info, c'est du hard delete <!-- bouton "Supprimer" dans le header SessionDetailPage, dialog de confirmation explicite. SupprimerSessionUseCase scanne les sous-dossiers de format (et email pour Numerique), supprime les fichiers matchant les slugs des acheteurs, supprime les commandes, supprime la session via SessionRepository.delete. Le dossier source reste intouchable. -->
- [x] il faut pouvoir archiver une session (avec confirmation), elle n'est plus dans la liste active, ca nettoie le dossier d'export (pour gagner de la place), mais ca garde l'état des commandes et autre data. plus rien n'est modifiable <!-- bouton "Archiver" sur SessionDetailPage avec dialog de confirmation, ArchiverSessionUseCase nettoie via le helper partagé nettoyerExportSession (réutilisé par SupprimerSession). Session.archivee + Session.assertModifiable propagé dans toutes les méthodes mutantes + tous les use cases qui touchent une commande. SessionsPage : sections "Actives" et "Archivées" séparées (archivée masquée si vide). Sur la page d'une session archivée : bandeau visible + boutons d'action masqués + bouton "Désarchiver" qui flip le flag (les fichiers ne reviennent pas). -->